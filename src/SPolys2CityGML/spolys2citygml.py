...
def convert():
  ...
  if len(sys.argv) != 2:
      print 'usage: spolys2acitygml.py FOLDERNAME'
      sys.exit()
  ...
  for f in dFiles:
      ...
      if str_firstline[0] == '#':
          ...
          if str_buildingid == 'NonID':
            #try to use the filename
            str_buildingid = f.split()[0]
...
def match_surface_type(str_polygontype):
    ...
    elif str_polygontype == 'OUTER_BUILDING_FLOOR_SURFACE':
        return 'OuterFloorSurface'
    elif str_polygontype == 'OUTER_BUILDING_CEILING_SURFACE':
...
if __name__ == "__main__":
  ...
  print '<!-- CityGML Dataset created with the spolys2citygml -->'
